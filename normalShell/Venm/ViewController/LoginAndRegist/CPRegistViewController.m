...
-(void)reloadUIAfterData
{
    ...
    if (_hasNeedInviteCode == 0) {
        _topView.height = 44*3;
        _referrerId.hidden = YES;
    }else if (_hasNeedInviteCode== 1){
        _referrerId.placeholder = @"请输入邀请码（可不填）";
    }else if (_hasNeedInviteCode== 2){
        _referrerId.placeholder = @"请输入邀请码";
    }
    ...
    [SUMRequest startWithDomainString:[CPGlobalDataManager shareGlobalData].domainUrlString
           ...
           completionBlockWithSuccess:^(__kindof SUMRequest *request) {
               ...
               if (request.resultIsOk) {
                   ...
                   _hasNeedInviteCode = [[preInfo DWStringForKey:@"inviteCode"]intValue];
...
- (IBAction)buttonActions:(UIButton *)sender {
    ...
    switch (sender.tag) {
        ...
        case 103:
        {
            ...
            }else if (_hasNeedInviteCode == 2 && _referrerId.text.length == 0){
                alertMsg = @"请输入邀请码号码";
            }
