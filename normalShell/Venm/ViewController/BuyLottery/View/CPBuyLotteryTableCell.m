...
@interface CPBuyLotteryTableCell ()
{
    IBOutlet UIImageView *_pictureImageView;
    IBOutlet UILabel *_titleLabel;
    
    IBOutlet UILabel *_nowLotteryNumberLabel;
    IBOutlet CPLotteryShowResultView *_nowLotteryAnswerView;
...
@implementation CPBuyLotteryTableCell
                         ...
                         cellWidth:(CGFloat)cellWidth;
{
    ...
    height = 50 + [CPLotteryShowResultView resultViewHeightByResult:openResult resultType:CPLotteryResultTypeByTypeString(lotteryModel.type) isWaitOpenResult:lotteryModel.lastOpen.length==0?YES:NO maxWidth:resultViewMaxWidth isList:YES];
    return height;
}
...
{
    NSTimeInterval distance = [_lotteryModel.endTime doubleValue]/1000 -([[NSDate date] timeIntervalSince1970]-[CPTimeManager shareTimeManager].beijingTiemDistance);
    ...
    NSString *ourString = intOur>9?[NSString stringWithFormat:@"%ld",(long)intOur]:[NSString stringWithFormat:@"0%ld",(long)intOur];
    NSString *minString = intMin>9?[NSString stringWithFormat:@"%ld",(long)intMin]:[NSString stringWithFormat:@"0%ld",(long)intMin];
    NSString *secondString = intSecond>9?[NSString stringWithFormat:@"%ld",(long)intSecond]:[NSString stringWithFormat:@"0%ld",(long)intSecond];
...
-(void)setLotteryModel:(CPLotteryModel *)lotteryModel
{
    ...
    [_nowLotteryAnswerView showResult:lastOpenResult resultType:CPLotteryResultTypeByTypeString(_lotteryModel.code) isWaitOpen:_lotteryModel.lastOpen.length==0?YES:NO];
    _nextLotteryDesLabel.text = [NSString stringWithFormat:@"距%@期 截止还有",_lotteryModel.period];
