...
#import "CPBuyLotteryCustomItem.h"

@interface CPBuyLotterySectionView ()
{
...
@implementation CPBuyLotterySectionView
...
-(NSArray *)heXiaoPlayDetailList
{
    if (!_heXiaoPlayDetailList) {
        _heXiaoPlayDetailList = [self.delegate heXiaoPlayDetailInfoList];
    }
...
-(CGFloat)itemHeight
{
    if (self.betType == CPBuyLotteryBetFast) {
        if ([CPBuyLotteryManager shareManager].currentBuyLotteryType == CPLotteryResultForK3) {
    ...
    for (int r=0 ; r<row; r++) {
        ...
        for (int l=0; l<line; l++) {
            ...
            if (self.betType == CPBuyLotteryBetForHeXiao) {
                ...
                [CPBuyLotteryFastItem addBuyLotteryFastItemOnView:self.contentView withFrame:frame buyInfo:info detailLabelTextColor:kCOLOR_R_G_B_A(109, 109, 109,1) clickAction:^(CPBuyLotteryFastItem *item, NSDictionary *buyInfo, BOOL isSelected) {
                    if (isSelected) {
                        /*
                         */
                        if (self.heXiaoArray.count>=11) {
                            CPBuyLotteryFastItem *firstItem = [self.heXiaoArray objectAtIndex:0];
                    ...
                    [self.delegate buyHeXiaoWithInfos:self.heXiaoArray value:[NSString stringWithFormat:@"%.2f",_heXiaoBonuds] playId:playId];
                
                }];
                
            }else if (self.betType == CPBuyLotteryBetFast) {
                
                [CPBuyLotteryFastItem addBuyLotteryFastItemOnView:self.contentView withFrame:frame buyInfo:info clickAction:^(NSDictionary *buyInfo, BOOL isSelected) {
                    [self.delegate buyLotteryBetFastBuyInfo:buyInfo sectionName:[self.buySectionInfo DWStringForKey:@"name"] isBuy:isSelected];
                }];
                
            }else if (self.betType ==CPBuyLotteryBetForLianXiaoWei) {
                
                [CPBuyLotteryFastItem addBuyLotteryFastItemOnView:self.contentView withFrame:frame buyInfo:info infoIndex:infoIndex clickAction:^(NSString *infoIndex, BOOL isSelected) {
                ...
                [CPBuyLotteryCustomItem addBuyLotteryCustomItemOnView:self.contentView withFrame:frame buyInfo:info clickAction:^(NSDictionary *buyInfo, NSString *value) {
                    [self.delegate buyLotteryBetCustomBuyInfo:buyInfo sectionName:[self.buySectionInfo DWStringForKey:@"name"] value:value];
                }];
...
-(void)buildLHCHorizontalViewStyleItems
{
    ...
    for (int i = 0; i<infos.count; i++) {
        ...
        [CPBuyLotteryCustomItem addHorizontalBuyLotteryCustomItemOnView:self.contentView withFrame:frame buyInfo:info clickAction:^(NSDictionary *buyInfo, NSString *value) {
            [self.delegate buyLotteryBetCustomBuyInfo:buyInfo sectionName:[self.buySectionInfo DWStringForKey:@"name"] value:value];
        }];
...
//        heXiaoCount
        ...
        _heXiaoBonuds = [[_heXiaoPlayInfo DWStringForKey:@"bonus"]doubleValue];
    ...
    for(int i = n; i >= k; i--)
    {
        ...
        else
        {
            ...
            for (int i = 0; i<indexs.count; i++) {
                ...
                CGFloat bonus = [[[[CPBuyLotteryManager shareManager]fetchDoubleFacePlayBounsByPlayId:[info DWStringForKey:@"playId"]]jdM]doubleValue];
                if (minBonus>bonus) {
                    minBonus = bonus;
