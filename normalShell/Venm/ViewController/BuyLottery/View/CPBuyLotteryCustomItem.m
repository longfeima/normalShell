...
@interface CPBuyLotteryCustomItem ()<UITextFieldDelegate>
...
@implementation CPBuyLotteryCustomItem
                 ...
                 clickAction:(CPBuyLotteryCustomItemFinishedAction)action
{
    ...
    if (self) {
        ...
        NSString *bounus = [NSString stringWithFormat:@"%@",[[[CPBuyLotteryManager shareManager]fetchDoubleFacePlayBounsByPlayId:[self.buyInfo DWStringForKey:@"playId"]]jdM]];
        NSString *haoString = [SUMUser shareUser].buyLotteryDetailHasNumberPan?@"号":@"";
        haoString = ([CPBuyLotteryManager shareManager].currentBuyLotteryType == CPLotteryResultForK3 && [[CPBuyLotteryManager shareManager].currentPlayKindDes isEqualToString:@"点数"])?@"点":@"";
            ...
            if (![[CPBuyLotteryManager shareManager].currentPlayKindDes isEqualToString:@"点数"])
            {
                ...
                if ([playName isPureInt]) {
                    ...
                    for (NSInteger i = 0; i<playNameCount; i++) {
                        ...
                        imgView.image = [CPBuyLotteryManager k3BackgroundImageByNumber:number];
...
{
    CPBuyLotteryCustomItem *item = [[CPBuyLotteryCustomItem alloc]initWithFrame:frame buyInfo:buyInfo clickAction:acion];
                           ...
                           clickAction:(CPBuyLotteryCustomItemFinishedAction)action
{
    ...
    if (self) {
        ...
        bounusLabel.text = [[[CPBuyLotteryManager shareManager]fetchDoubleFacePlayBounsByPlayId:[self.buyInfo DWStringForKey:@"playId"]]jdM];
...
+(void)addHorizontalBuyLotteryCustomItemOnView:(UIView *)supView
                                   ...
                                   clickAction:(CPBuyLotteryCustomItemFinishedAction)acion
{
    CPBuyLotteryCustomItem *item = [[CPBuyLotteryCustomItem alloc]initWithHorizontalFrame:frame buyInfo:buyInfo clickAction:acion];
    [supView addSubview:item];
}
