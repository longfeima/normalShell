...
- (IBAction)goToBetAction:(UIButton *)sender {
    ...
    if (_typeList.count<=_selectedTypeIndex) {
        return;
    }
    NSDictionary *info = _typeList[_selectedTypeIndex];
    /*
     {
     code = jsssc;
     name = "\U6781\U901f\U65f6\U65f6\U5f69";
     num = 53;
     }
     */
    [[CPBuyLotteryManager shareManager]pushToBuyLtyDetailVCFromNav:self.navigationController ltyName:[info DWStringForKey:@"name"] ltyCode:[info DWStringForKey:@"code"] ltyNum:[info DWStringForKey:@"num"]];
}
