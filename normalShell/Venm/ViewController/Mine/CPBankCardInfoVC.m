...
@interface CPBankCardInfoVC ()<UITextFieldDelegate,UIScrollViewDelegate, UITextFieldDelegate>
{
...
- (void)viewDidLoad {
    ...
    if (_bankInfo) {
        ...
        _provinceTf.text = [_bankInfo DWStringForKey:@"addr"].length>0?[_bankInfo DWStringForKey:@"addr"]:nil;
    ...
    _payPasswordTf.keyboardType = UIKeyboardTypeNumberPad;
    _payPasswordTf.delegate = self;
}
...
- (IBAction)confirmAction:(UIButton *)sender {
    ...
    if (_provinceTf.text.length == 0) {
        [SVProgressHUD way_showInfoCanTouchWithStatus:@"请输入开户行地址" dismissAfterInterval:2 onView:self.navigationController.view];
        return;
    }
...
-(void)queryBindBankInfo
{
    ...
    [paramsDic setObject:_provinceTf.text forKey:@"addr"];
