...
- (void)viewDidLoad {
    ...
    if ([[_payInfo DWStringForKey:@"needDown"]intValue] == 1) {
        if ([[_payInfo DWStringForKey:@"isSixFour"]intValue] == 1) {
            ...
            UIImage *img = [UIImage imageWithData:[base64String base64DecodedData]];
            
            if (img) {
                CGFloat imgViewWidth = img.size.width * _qrCodeImageView.height/img.size.height;
            ...
            [_qrCodeImageView sd_setImageWithURL:[NSURL URLWithString:[_payInfo DWStringForKey:@"payUrl"]] placeholderImage:nil options:SDWebImageRetryFailed completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
                if (image) {
