...
- (void)viewDidLoad {
    ...
    _switch.on = [SUMUser shareUser].tokenInfo.isDefaultGfPlay;
    ...
    if ([CPGlobalDataManager shareGlobalData].isReviewVersion) {
        ...
        _dataList = @[@[@"登录密码",@"密保问题",@"提款密码",@"我的银行卡"],@[@"是否默认官方玩法"],@[@"关于我们"]];
        _imageList = @[@[@"shezhi_03_goucai_mine",@"geren_tubiao_13_goucai_mine",@"shezhi_03_goucai_mine",@"shezhi_04_goucai_mine"],@[@"setting_guanfangwanfa"],@[@"geren_tubiao_30_goucai_mine"]];
    }
...
- (IBAction)gfSettingAction:(UISwitch *)sender {
    
    @synchronized(self){
        
        BOOL isOn = sender.isOn;
        [self querySetGfPlayDefault:isOn];
    }
...
-(void)querySetGfPlayDefault:(BOOL)isOpen
{
    ...
    [SUMRequest startWithDomainString:[CPGlobalDataManager shareGlobalData].domainUrlString
           ...
           completionBlockWithSuccess:^(__kindof SUMRequest *request) {
               ...
               if (request.resultIsOk) {
                   ...
                   [SUMUser shareUser].tokenInfo.isOrg = isOpen?@"1":@"0";
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    }else if([title isEqualToString:@"是否默认官方玩法"]){
