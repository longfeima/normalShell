...
{
    [self goToLoginViewControllerWithMsg:@""];
}

-(void)goToLoginViewControllerWithMsg:(NSString *)msg
{
    msg = msg.length>0?msg:@"请先登录";
    [SVProgressHUD way_showInfoCanTouchAutoDismissWithStatus:msg];
    CPLoginViewController *loginVC = [CPLoginViewController new];
    UINavigationController *nav = [[UINavigationController alloc]initWithRootViewController:loginVC];
    [self presentViewController:nav animated:YES completion:nil];
}
...
-(void)loginTimeOutGoToLoginViewController
{
    if (!self.presentedViewController) {
        [self goToLoginViewControllerWithMsg:[SUMUser shareUser].tokenInfo.errorMsg];
    }
