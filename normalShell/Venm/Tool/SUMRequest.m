...
#pragma mark- 请求结果的全局处理

-(void)globalDealWithResult
{
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        dispatch_async(dispatch_get_main_queue(), ^{
            NSString *plVersion = [self.resultInfo DWStringForKey:@"plVersion"];
            [[SUMUser shareUser]addPlVersion:plVersion];
        });
    });
}
    ...
    [sumRequest startWithCompletionBlockWithSuccess:^(__kindof YTKBaseRequest * _Nonnull request) {
        ...
        if ([request isKindOfClass:[SUMRequest class]]) {
            ...
            if (sRequest.resultInfo) {
                NSInteger status = [[sRequest.resultInfo DWStringForKey:@"status"]integerValue];
                if (status==110) {
