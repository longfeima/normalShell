...
+(void)setObject:(id)obj forAttribute:(NSDictionary*)attribute
{
    ...
    NSArray *objPropertyList = [self p_getPropertyList:obj];
    ...
    @try
    {
            for (NSString* key in keys)
            {
                ...
                if ([objPropertyList containsObject:key]) {
                    @try {
...
+ (NSArray *)p_getPropertyList:(Class)cls {
    unsigned int outCount = 0;
    objc_property_t *c_p_l = class_copyPropertyList([cls class], &outCount);
    NSMutableArray *list = [NSMutableArray array];
    for (int i = 0; i < outCount; i++) {
        objc_property_t p = c_p_l[i];
        const char* c_f = property_getName(p);
        
        NSString *oc_p = [NSString stringWithUTF8String:c_f];
        [list addObject:oc_p];
    }
    free(c_p_l);
    return list;
