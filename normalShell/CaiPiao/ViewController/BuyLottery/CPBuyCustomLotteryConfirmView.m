...
@interface CPBuyCustomLotteryConfirmView ()
{
...
@implementation CPBuyCustomLotteryConfirmView

+(void)showCustomLotteryConfirmViewOnView:(UIView *)supView
                               lotterys:(NSArray *)lotterys
                          numberPeriods:(NSString *)numberPeriods
                                comfirm:(CPBuyCustomLotteryConfirmAction)confirm
{
    CPBuyCustomLotteryConfirmView *view = [CPBuyCustomLotteryConfirmView createViewFromNib];
          ...
          comfirm:(CPBuyCustomLotteryConfirmAction)confirm
{
    ...
    for (NSDictionary *info in lotterys) {
        NSInteger betValue = [[info DWStringForKey:@"betValue"]integerValue];
        NSInteger bouns = [[NSString stringWithFormat:@"%ld",(NSInteger)([[info DWStringForKey:@"bonus"]floatValue]*10)]integerValue];
        allPay += betValue;
        award = award+(betValue *bouns)/10.0f;
        win = award - allPay;
    }
